...
public class CampanaActivity extends AppCompatActivity {
    ...
    private void asignarComponentes() {
        // Asignamos el audio
        mp = MediaPlayer.create(this, R.raw.campana_sobre_campana);

        prefs = getSharedPreferences("MisPrefs", MODE_PRIVATE);

        mp.start();
        Toast.makeText(getApplicationContext(), "Reproduciendo: Campana Sobre Campana", Toast.LENGTH_SHORT).show();
    }

    public void guardarFav(View view) {
        prefs.edit().putString("villancicoFav", "Campana Sobre Campana").apply();
